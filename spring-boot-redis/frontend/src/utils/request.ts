...
const request: AxiosInstance=axios.create({
    baseURL: "http://localhost:8082/api", // API 服务器地址
...
request.interceptors.response.use(  
    (response: AxiosResponse<ApiResponse>) => {
        ...
        switch (error.response.status) {
            case 401:
                message="未授权，请重新登录";
                // 401 表示 token 无效或过期，清除本地存储的认证信息并跳转到登录⻚
                ...
                case 404:
                    ...
                    break;
                case 500:message="服务器内部错误"; // 服务器错误
                    break;
                case 502:message="网关错误"; // 网关或代理服务器错误
                    break;
                case 503:message="服务不可用"; // 服务暂时不可用
                    break;
                case 504:
                    message="网关超时"; // 网关超时
                    break;
