      ...
      />
        ...
        <el-form-item>
          <el-button 
            ...
            style="width: 100%"
          >
            <el-icon><Message/></el-icon>  {{ isCountingDown ? `${countdown}秒后重发` : "发送验证码" }}
          </el-button>
        </el-form-item>
...
const rules: FormRules= {
  ...
  code:
   [  
    ...
    { len: 4, message: "验证码⻓度为4位", trigger: "blur" },  
  ],
};
const sendSms=async () => {
...
const login=async () => {
  ...
  try {
    ...
    const errorMsg="请求失败，请稍后再试。";errorMessage.value=errorMsg;
    ElMessage.error(errorMsg);  
  } finally {
...
.login-container {
  ...
  background: linear-gradient(135deg, #667eea 0%, #764ba210 0%);
  padding :20px;
  }
...
.login-card {
  width: 100%;
  max-width: 400px;
  box-shadow: 015px35pxrgba(0, 0, 0, 0.1);
  border-radius: 15px;
  overflow: hidden;
  }
...
:deep(.el-button:hover) {
  ...
  box-shadow: 06px20pxrgba(0, 0, 0, 0.15);
  }
:deep(.el-alert) {
  border-radius: 8px;
  margin-top: 10px;
...
:deep(.el-card__header) {
    background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe210 0%);
